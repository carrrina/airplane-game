...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 4.5f, 0));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_F)) {
			negativeFoV += deltaTime;
			projectionMatrix = glm::perspective(RADIANS(60) - negativeFoV, window->props.aspectRatio, 0.01f, 200.0f);
		}
		...
		if (window->KeyHold(GLFW_KEY_G)) {
			dim += deltaTime;
			projectionMatrix = glm::ortho(-dim, dim, -dim, dim, 0.0f, 200.0f);
		}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_P)
	{
		projectionMatrix = glm::perspective(negativeFoV * RADIANS(60), window->props.aspectRatio, 0.01f, 200.0f);
	}
}
