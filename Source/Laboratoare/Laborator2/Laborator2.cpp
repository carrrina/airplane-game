...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<VertexFormat> verticesS
		{
			...
			VertexFormat(glm::vec3(2, 2, 4), glm::vec3(1, 0, 0)),
			VertexFormat(glm::vec3(2, 4, 4), glm::vec3(0, 1, 0)),
			VertexFormat(glm::vec3(2, 4, 2), glm::vec3(0, 0, 0)),
		};
		...
		Mesh* square = CreateMesh("square", verticesS, indicesS);
			...
			//vector<unsigned short> indicesC;

			//double i;
			//for (i = 0; i <= 100; i += 0.1) {
			//	vertices.push_back(VertexFormat(glm::vec3(i / 10, sin(i), cos(i)), glm::vec3(0, i / 100, 1 - i / 100)));
			//	indices.push_back((int)(i * 10));
			//}

			//// Create a new mesh from buffer data
			//Mesh* cerc = CreateMesh("cerc", verticesC, indicesC);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	// render an object using colors from vertex
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.25f));
	//RenderMesh(meshes["cerc"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.25f));
